...
def get_samples(args, images, bboxes, cl, size, class_map, job):
    '''
    Input : Data unit images and object bounding boxes
    Output : Bounding box cropped images (+ negative background 
             patches depending on job), class labels, 
    ...
    for row in range(images.shape[0]):
        ...
        if job == 'train': 
            while (True): 
                #get the negative bounding box from the image outside the original bounding box but with some overlap range
                #randomly select 4 points within a window specified by rendered_img_size and e
                amin = random.randint(0, rendered_img_size - e)
                bmin = random.randint(0,rendered_img_size - e)
                #Adding the resize buffer to prevent resizing errors on very thin strips
                ...
                if 0.5 < a_ratio and a_ratio < 2:
                    if (overlap(x_min, y_min, x_max, y_max, amin, bmin, amax, bmax, ov_range)):
                        break
...
def overlap(xmin, ymin, xmax, ymax, amin, bmin, amax, bmax, ov_range):
    """
    Checks for the rectangular collision for the two boxes with a certain overlap range.
    Returns True if the two boxes are valid and do not overlap more than the allowed range
    """
